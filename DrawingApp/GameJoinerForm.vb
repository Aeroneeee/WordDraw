    ...
    Async Function getData(id) As Task
        ...
        For Each kvp As KeyValuePair(Of String, String) In dicBody
            ...
            If kvp.Key = "y_coordinate" Then
                ...
                'Console.WriteLine(kvp.Value)
            End If
            If kvp.Key = "xEnd_coordinate" Then
                xEnd = Integer.Parse(kvp.Value)
                'Console.WriteLine(kvp.Value)
            End If
            If kvp.Key = "yEnd_coordinate" Then
                yEnd = Integer.Parse(kvp.Value)
                'Console.WriteLine(kvp.Value)
            End If

            If x_coordinate <> 0 And y_coordinate <> 0 And yEnd <> 0 And xEnd <> 0 Then
                'Console.WriteLine()
